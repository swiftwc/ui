...
export function queryInsertPosition(frame?: Components.BodyView | Components.SheetView) {
...
export function queryFrameToolbars(sv?: Components.ScrollView) {
  const isSidebarWrapped = sv?.parentElement?.matches('dialog[is=sidebar-view]'), // boolean
    scene = (isSidebarWrapped ? sv?.parentElement : sv) ?? undefined, // scroll-view | sidebar-view
    frame = scene?.parentElement ?? undefined, // body-view | sheet-view | nav-stack | nav-split-view
    toolbarElements = sv?.parentElement?.querySelectorAll(
      `:scope > tool-bar > tool-bar-item,:scope > tool-bar > tool-bar-item-group` //`:scope > navigation-bar > tool-bar-item,:scope > bottom-bar > tool-bar-item,:scope > navigation-bar > tool-bar-item-group,:scope > bottom-bar > tool-bar-item-group` // tool-bar-item[] | tool-bar-item-group[]
    )
