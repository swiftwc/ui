...
/**
 * @summary A control that starts an action. Styled with a standard border, adapting to the button’s surroundings.
 *
 * @example <button is="bordered-button"><label-view system-image="hand-tap" title="Tap Me"></label-view></button> — Creating a bordered button
 *
 * @slot overlay
 *
 */
export class BorderedButton extends ButtonBase {
  ...
  static polyfillAttributeChangedCallback([{ attributeName, target, oldValue }]: Pick<MutationRecord, 'attributeName' | 'oldValue' | 'target'>[]) {
    debug(`${BorderedButton.name} ⚡️ attr-change [${attributeName}] ("${oldValue}" → "${(target as HTMLElement).getAttribute(attributeName ?? '')}")`)
